...
static const float TRANS_SPEED = 0.2;
static const float ANG_SPEED = 0.2*M_PI_2;
...
public:
    ...
    */
    ...
    void publish_lidar_odometry() { 
				std::cout << "publish lidar data" << std::endl;
        odometry_t lidar_odometry;
        lidar_odometry.x = x_lidar;
        lidar_odometry.y = y_lidar;
        lidar_odometry.theta = theta_lidar;
        lidar_odometry.utime = now();
        lcmInstance->publish("LIDAR POSE", &lidar_odometry);
    }
    ...
    void handleLidar(const lcm::ReceiveBuffer* buf, const std::string& channel, const lidar_t* lidar) {
        ...
        if (state_ == DRIVE) {
            ++lidar_counter;
            if (fabs(min_dist_side - min_dist_front) < 0.03) {
                state_ = TURN;
                ...
                lidar.x = x_lidar;
                lidar.y = y_lidar;
                lcmInstance->publish("TURN CHANNEL", &lidar);
                ++lidar_state;
                theta_lidar = ( theta_lidar + M_PI_2 );
                if ( theta_lidar > 2 * M_PI ) 
                {
                    theta_lidar -= 2 * M_PI;
                }
            } else if (lidar_counter > 5) {
            	if ( lidar_state % 4 == 0 )
            	{
                x_lidar = 1.5 - min_dist_front;
                y_lidar = min_dist_side - 0.5;
                std::cout << "s0 - " << min_dist_front << " " << min_dist_side << std::endl;
            	}
            	else if ( lidar_state % 4 == 1 )
            	{
                x_lidar = 1.5 - min_dist_side;
                y_lidar = 1.5 - min_dist_front;
                std::cout << "s1 - " << min_dist_front << " " << min_dist_side << std::endl;
            	}
            	else if ( lidar_state % 4 == 2 )
            	{
                x_lidar = min_dist_front - 0.5;
                y_lidar = 1.5 - min_dist_side;
                std::cout << "s2 - " << min_dist_front << " " << min_dist_side << std::endl;
            	}
            	else if ( lidar_state % 4 == 3)
            	{
                x_lidar = min_dist_side - 0.5;
                y_lidar = min_dist_front - 0.5;
                std::cout << "s3 - " << min_dist_front << " " << min_dist_side << std::endl;
            	}
