...
, lcmInstance_(lcmInstance)
{
    ...
    std::array<float, 3> startCube = {{0.0, 0.0, 0.0}};
    squares.push_back(startCube);
    
    // Copy over all the colors for drawing pose traces
    traceColors_.push_back(vx_blue);
...
int BotGui::onKeyEvent(vx_layer_t* layer, vx_key_event_t* event)
{
    ...
    else if(event->key_code == VX_KEY_BACKSPACE) 
    {
        reset_odometry_t reset;
        reset.x = 0;
        reset.y = 0;
        reset.theta = 0;
        std::cout << "rsetting odometry";
        lcmInstance_->publish("RESET_ODOMETRY", &reset);
    }
    return 0;
}
...
void BotGui::onDisplayStart(vx_display_t* display)
{
    ...
    lcmInstance_->subscribe("TURN CHANNEL", &BotGui::handleTurn, this);
		lcmInstance_->subscribe("LIDAR POSE", &BotGui::handleLidar, this);
}
...
void BotGui::render(void)
{
    ...
    vx_buffer_t* headingBuf = vx_world_get_buffer(world_, "heading");
    if (1) {
        float line[6] = { 0.0, 0.0, 0.0, 1.5, 0.0, 0.0 };
        vx_buffer_add_back(headingBuf, vxo_chain(vxo_mat_translate3(odometry_.x, odometry_.y, 0),
            vxo_mat_rotate_z(odometry_.theta),
            vxo_lines(vx_resc_copyf(line, 2*3), 2*3, GL_LINE_STRIP, vxo_points_style(vx_blue, 2.0f))));
    }
    vx_buffer_swap(headingBuf);

    vx_buffer_t* turnEvents = vx_world_get_buffer(world_, "turns");
    for (int i = 0; i < (int)squares.size(); ++i ) {
        std::array<float,3> turnLocation = squares.at(i);
        vx_buffer_add_back(turnEvents, vxo_chain(
            vxo_mat_translate3(turnLocation[0], turnLocation[1], 0),
            vxo_mat_scale3(0.1, 0.1, 0.1),
            vxo_rect(vxo_lines_style(vx_red, 3.0f))));
    }
    vx_buffer_swap(turnEvents);
...
void BotGui::handleLidar(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const odometry_t* odom)
{
    std::lock_guard<std::mutex> autoLock(vxLock_);
...
void BotGui::handleTurn(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const turn_t* turn) 
{
    std::lock_guard<std::mutex> autoLock(vxLock_);
    if ( turn->x == -1 && turn->y == -1 )
    {
        std::array<float,3> nextTurn = {{odometry_.x, odometry_.y, 0}};
        squares.push_back(nextTurn);
        std::cout << odometry_.x << " " << odometry_.y << std::endl;
    } 
    else 
    {
        std::array<float,3> nextTurn = {{turn->x, turn->y, 0}};
        squares.push_back(nextTurn);
...
void BotGui::addPose(const pose_xyt_t& pose, const std::string& channel)
{
    ...
    {
        std::cout << "Adding new PoseTrace:" << channel << 'with color ' << nextColorIndex_ << '\n';
        Trace trace;
