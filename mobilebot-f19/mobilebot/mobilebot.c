...
*******************************************************************************/
int main(){
    // TONY: Task 3 velocity setpoints
    mb_setpoints.manual_ctl = 0;  //  manual control, set left right setpoints directly
	...
	while(rc_get_state()!=EXITING){
        ...
        led_heartbeat();
        rc_nanosleep(7E8);
        // mb_motor_set(RIGHT_MOTOR, 0.6);
		// mb_motor_set(LEFT_MOTOR, 0.6);
	}
	rc_led_set(RC_LED_RED, LED_ON);
	// exit cleanly
...
void publish_mb_msgs(){
    ...
    sp_msg.utime = now;
    sp_msg.left_sp = mb_setpoints.left_sp;
    sp_msg.right_sp = mb_setpoints.right_sp;
    sp_msg.left_vel = mb_state.left_velocity;
    sp_msg.right_vel = mb_state.right_velocity;

    //publish IMU & Encoder & Motor command & Setpoint Data to LCM
    ...
    setpoint_t_publish(lcm, SETPOINT_CHANNEL, &sp_msg); // TONY: publish setpoints to LCM
}
...
void mobilebot_controller(){
    ...
    if(!mb_setpoints.manual_ctl){
        // printf("setpoint control, setpoint %f\n", mb_state.right_cmd);
        // mb_motor_set(RIGHT_MOTOR, mb_setpoints.right_sp); // for task 4
   		// mb_motor_set(LEFT_MOTOR, mb_setpoints.left_sp);
    	mb_motor_set(RIGHT_MOTOR, mb_state.right_cmd); // for task 5
    ...
    if(mb_setpoints.manual_ctl){
        ...
        mb_motor_set(RIGHT_MOTOR, mb_setpoints.right_sp);
   		mb_motor_set(LEFT_MOTOR, mb_setpoints.left_sp);
   	}

    mb_state.last_time = now;
}
...
void motor_command_handler(const lcm_recv_buf_t *rbuf, const char *channel,
    ...
    //printf("trans VELOCITY: %f\n", msg->trans_v);
    //printf("FORWARD VELOCITY: %f\n", mb_setpoints.fwd_velocity);

    if(fabs(mb_setpoints.left_sp - msg->left_motor_speed) > 0.01){
        printf("REINIT LEFT %f, %f\n", mb_setpoints.left_sp, msg->left_motor_speed);
        mb_initialize_left();
    }
    if(fabs(mb_setpoints.right_sp - msg->right_motor_speed) > 0.01){
        printf("REINIT RIGHT %f, %f\n", mb_setpoints.right_sp, msg->right_motor_speed);
        mb_initialize_right();
    }

    // printf("Motor command %f\n", msg->left_motor_speed);
    // mb_setpoints.left_sp = msg->left_motor_speed; // todo: task 4 only
    // mb_setpoints.right_sp = msg->right_motor_speed;
}
...
void* setpoint_control_loop(void* ptr){
    ...
    while (1){
        /*
        I don't believe we ever use a Spektrum radio so just gonna comment this bs out
        if (rc_dsm_is_new_data()){
            ...
            if (rc_dsm_ch_normalized(5) > 0.0){
                mb_setpoints.manual_ctl = 1; // TODO TONY: temorarily disabling this for task 3
